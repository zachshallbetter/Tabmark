...
/* Diagram of localStorage/tasks: 

localStorage: 
{
	default: {
		lastUpdated: 29058704,
		tasks: [
		    { 
		        name: "hi", 
	...
	user1: { 
		...
		tasks: [
			{
		        name: "hi3", 
		        tabs: {"g": "http://g.com", "h": "http://h.com", "j": "http://j.com"}, 
		        update: false		
		    }
		]
	}
...
window.onload = function() {
	if (localStorage['cloud-enabled'] == undefined) {
		localStorage['cloud-enabled'] = true;
	}
	useDefault();
	var getUrls = function() {
		$.ajax(WEBSERVICE_URL + "/login", {
			success: function(data) {
				LOGIN_URL = /* WEBSERVICE_URL + */ data.login;
				LOGOUT_URL = /* WEBSERVICE_URL + */ data.logout;

				loginAttempt();
			},
			error: function(data) {
				setTimeout(getUrls, 20000);
			}
		});
	};
	getUrls();
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
	if (request.type == "load_task") {
		...
		user.logout = LOGOUT_URL;
		sendResponse(user);
	} else if (request.type == "set_user") {
		user = request.user;
		persistLocal(request.immediate);
		...
		$.ajax(WEBSERVICE_URL, {
			...
			success: function(data) {
				...
				if (localStorage[data]) {
					user = $.parseJSON(localStorage[data]);
					user.tasks = $.parseJSON(sjcl.decrypt(key, user.tasks));
				} else
...
var windowId = tabs[tabId];
	if (activeTasks[windowId]) {
		if (removeInfo.isWindowClosing) {
			...
			var task = user.tasks[indexOfTask(user.tasks, taskName)];
			if (task && task.update) {
...
function saveUser() {
	if (!key)
		key = SHA1(user.email);
	oldTasks = user.tasks
	user.tasks = sjcl.encrypt(key, JSON.stringify(user.tasks || ''));
...
function persistUser() {
	if (localStorage['cloud-enabled']) {
		$.post(WEBSERVICE_URL, {
			tasks: sjcl.encrypt(key, JSON.stringify(user.tasks))
		}, function(data) {
			user.lastUpdated = parseFloat(data);
			saveUser();
		});
		cloudTimer = null;
	}
